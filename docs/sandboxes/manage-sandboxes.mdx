...
Docker and Podman gateways map a default `--gpu` request to one concrete NVIDIA
CDI device when individual CDI devices are available. On WSL2 all-only runtimes,
the default can fall back to `nvidia.com/gpu=all`. Use `--gpu-device` to pass an
explicit driver-native GPU device ID.
